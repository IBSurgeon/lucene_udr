...
Анализ - это преобразование занного текста в более мелкие и точные единицы для облегчения поиска.
...
Список доступных анализаторов можно узнать с помощью процедуры `FTS$MANAGEMENT.FTS$ANALYZERS`.
...
Такие анализаторы разделяют текст на слова, числа, URL адреса и EMAIL. Они преобразует текст в нижний регистр, после чего к 
...
Основа слова не обязательно совпадает с морфологическим корнем слова. Стемминг необходим для того чтобы поиск мог осуществляться не только по самому слову, но и его формам.
...
Библиотека IBSurgeon FTS UDR позволяет создавать свои собственные анализаторы. Через язык SQL невозожно изменить алгоритмы разделения текста на термы и алгоритмы стемминга, 
однако вы можете задать список своих стоп слов.

Создать свой анализатор можно на основе одного из встроенных анализаторов. Для того чтобы было возмжно создать собственный анализатор на основе базового, 
базовый анализатор должен поддерживать фильтр стоп слов. Новый анализатор создаётся с пустым списком стоп слов.

Для создание собственного анализатора вызовете процедуру `FTS$MANAGEMENT.FTS$CREATE_ANALYZER`. Первым параметром указывается имя нового анализатора, вторым - имя базового анализатора,
...
Процедура `FTS$MANAGEMENT.FTS$ANALYZER_STOP_WORDS` возращает список стоп слов для заданного анализатора.

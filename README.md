...
- FTS$LIMIT - ограничение на количество записей (по умолчанию 1000).
...
### Процедура FTS$INSERT_RECORD

Процедура `FTS$INSERT_RECORD` добавляет ссылку на запись таблицы во все полнотекстовые индексы, которые построены для этой таблицы.
...
### Процедура FTS$UPDATE_RECORD

Процедура `FTS$UPDATE_RECORD` обновляет данные во всех полнотекстовых индексах, построенных для этой таблицы, на которые ссылается запись с указателем FTS$DB_KEY.
...
### Процедура FTS$DELETE_RECORD

Процедура `FTS$DELETE_RECORD` удаляет данные во всех полнотекстовых индексах, построенных для этой таблицы, на которые ссылается запись с указателем FTS$DB_KEY.
...
2. Поддерживать полнотекстовые индексы можно онлайн с помощью триггеров и вызова процедур `FTS$INSERT_RECORD`, `FTS$UPDATE_RECORD` и `FTS$DELETE_RECORD`.
Этот метод существенно замедляет модификацию данных в таблицах, для которых построены полнотекстовые индексы.
